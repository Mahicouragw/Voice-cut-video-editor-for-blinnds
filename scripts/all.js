...
async function publish(){
  ...
  await git(['-c','user.name=VoiceCut deployment automation','-c','user.email=voicecut-deployment@users.noreply.github.com','commit','-m','VoiceCut app 2.5: full-coverage caption review, fast silence preview with progress, completion notifications']);
  ...
  fs.writeFileSync(body,'User-authorized VoiceCut app 2.5 update.\n\nCaptions now report full coverage (which minutes have speech), warn when the chosen spoken language mismatches detection, preview the first caption, and review with Previous/Next, a counter, and Read-all-aloud. Silence removal gains a fast Detect-gaps preview on a small audio copy, skips re-encoding when no gaps exist, uses a faster encoder preset, and shows a real progress bar with an elapsed timer. Long jobs (captions, cleanup, silence, export) can send completion notifications when the app is in the background, permission-gated in Settings with an Android app bridge. No keys in the app; keys stay in server env.\n\nNo API keys are included. This automation merges only after the website and Android pull-request workflows succeed. It does not bypass protected-branch rules or delete branches.\n');
  await run(gh,['pr','create','--repo',REPO,'--base','main','--head',branch,'--title','VoiceCut app 2.5: caption review + silence progress + completion notifications','--body-file',body]);
