...
async function publish(){
  ...
  await git(['-c','user.name=VoiceCut deployment automation','-c','user.email=voicecut-deployment@users.noreply.github.com','commit','-m','VoiceCut app 2.6: buried-voice rescue, failure notifications, audio merge plus reverse, video reverse']);
  ...
  fs.writeFileSync(body,'User-authorized VoiceCut app 2.6 update.\n\nNoise reduction now rescues buried voice: steady drone is notched first, voiced peaks are restored so the voice never sounds reduced, and when engine-like noise dominates, a rescue blend keeps the voice clearly audible instead of going silent. Failures and mid-job stops also notify, not just completions. New editing tools: reverse selected clip audio, merge unmuted audio clips into one (both undoable, on-device), and reverse video up to 2 minutes on the server with memory-safe chunking. No keys in the app; keys stay in server env.\n\nNo API keys are included. This automation merges only after the website and Android pull-request workflows succeed. It does not bypass protected-branch rules or delete branches.\n');
  await run(gh,['pr','create','--repo',REPO,'--base','main','--head',branch,'--title','VoiceCut app 2.6: voice rescue + failure notifications + merge reverse tools','--body-file',body]);
