...
async function publish(){
  ...
  fs.writeFileSync(body,'User-authorized VoiceCut 2.0 update.\n\nIncludes a tested home/library/settings shell with an editor that opens only with a project, multi-project library with rename, working crop/rotate/freeze-frame export, Voice Focus noise reduction, and automatic cloud AI (free Groq/Deepgram/AssemblyAI captions, keyless DeepFilterNet denoising, optional ElevenLabs isolation) with no server URL, API key, or provider choice in the normal user UI. Backend credentials stay in server environment variables; a hidden session-only page-address override exists for the owner’s own testing.\n\nNo API keys are included. External provider quality still requires a short live test with the owner’s own keys. Batch cleanup remains unavailable by design (one clip at a time with preview).\n\nThis automation merges only after the website and Android pull-request workflows succeed. It does not bypass protected-branch rules or delete branches.\n');
  await run(gh,['pr','create','--repo',REPO,'--base','main','--head',branch,'--title','VoiceCut 2.0: accessible shell, project library, crop/rotate/freeze export, automatic cloud AI','--body-file',body]);
