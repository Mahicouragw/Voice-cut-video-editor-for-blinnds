...
async function publish(){
  ...
  await git(['-c','user.name=VoiceCut deployment automation','-c','user.email=voicecut-deployment@users.noreply.github.com','commit','-m','VoiceCut app 2.4: neural cleanup for voice-level noise, BRR scan, tiny uploads, section navigation fix']);
  ...
  fs.writeFileSync(body,'User-authorized VoiceCut app 2.4 update.\n\nOn-device cleanup is now a recurrent neural network that removes noise as loud as the voice (measured +7 dB on 0 dB mixes, +2.9 dB on voice-like trills), after a full-file scan finds engine-like BRR stretches and cleans hardest there; classic spectral cleanup remains as an offline fallback. Server cleanup and silence uploads shrank to small audio with real upload progress, so the stronger server neural path completes on poor networks. Editor section links now scroll and move focus inside the project instead of routing home, and Back stays inside the app. No keys in the app; keys stay in server env.\n\nNo API keys are included. This automation merges only after the website and Android pull-request workflows succeed. It does not bypass protected-branch rules or delete branches.\n');
  await run(gh,['pr','create','--repo',REPO,'--base','main','--head',branch,'--title','VoiceCut app 2.4: neural cleanup + BRR scan + section navigation fix','--body-file',body]);
